...
def player_worker_decode(
    ...
    while not quit_event.is_set():
        try:
            ...
            if isinstance(exc, av.error.EOFError):
                start_time = time.time()
                container.seek(0)
                continue
        ...
        if throttle_playback:
            ...
            if frame_time and frame_time > elapsed_time + 0.5:
                time.sleep(0.1)
...
def player_worker_demux(
    ...
    while not quit_event.is_set():
        try:
            ...
            logger.exception("Exception on playing file")
            if isinstance(exc, av.FFmpegError) and exc.errno == errno.EAGAIN:
                time.sleep(0.01)
                continue
            if isinstance(exc, StopIteration) and loop_playback:
...
class MediaPlayer:
    ...
    def __init__(
        ...
        logger.info(f"Throttle playback: {self._throttle_playback}")
